modules.report.sheets <- function (env, spot.list, main, path)
{
    ...
    if (main != "Correlation Cluster" && main != "K-Means Cluster") {
        ...
        par(mar = c(5, 4, 4, 1))
        col <- if (main != "D-Cluster")
            env$color.palette.portraits(1000)
        else colorRampPalette(c("blue2", "white", "red2"))(1000)
        image(matrix(spot.list$overview.map, env$preferences$dim.1stLvlSom,
            env$preferences$dim.1stLvlSom), axes = FALSE, col = col,
    ...
    image(x = c(1:env$preferences$dim.1stLvlSom), y = c(1:env$preferences$dim.1stLvlSom),
        z = matrix(spot.list$overview.mask, env$preferences$dim.1stLvlSom,
            env$preferences$dim.1stLvlSom), col = colorRampPalette(c("darkblue",
            "blue", "lightblue", "green2", "yellow", "red", "darkred"))(max(spot.list$overview.mask,
            na.rm = TRUE)), axes = TRUE, main = main, cex.main = 1.5,
        xlab = "", ylab = "", las = 1)
    mtext("annotation", 3)
    spot.borders(spot.list)
    box()
    par(new = TRUE)
    points(do.call(rbind, lapply(spot.list$spots, function(x) {
        x$position
    })), pch = 16, cex = 3, col = "black")
    points(do.call(rbind, lapply(spot.list$spots, function(x) {
        x$position
    })), pch = 1, cex = 3, col = "white")
    text(do.call(rbind, lapply(spot.list$spots, function(x) {
        x$position
    })), names(spot.list$spots), col = "white")
    par(mar = c(5, 1, 4, 2))
    plot(0, type = "n", axes = FALSE, xlab = "", ylab = "", xlim = c(0,
    ...
    if (env$preferences$activated.modules$geneset.analysis) {
        ...
        top.GS <- lapply(spot.list$spots, function(x) names(head(x$Fisher.p,
            n.sets)))
        leg.col <- colorRampPalette(c("darkblue", "blue", "lightblue",
            "green2", "yellow", "red", "darkred"))(length(spot.list$spots))
        leg.col <- as.vector(sapply(leg.col, c, rep(NA, n.sets -
            1)))
    ...
    sample.spot.expression.image <- if (nrow(spot.list$spotdata) >
        1) {
    ...
    layout(matrix(c(0, 2, 0, 3, 1, 0, 0, 4, 5), 3, 3), heights = c(0.8,
        6, 2), widths = c(0.5, 5, 3))
    par(mar = c(0, 0, 0, 0))
    image(1:ncol(env$indata), 1:nrow(spot.list$spotdata), sample.spot.expression.image,
        col = env$color.palette.heatmaps(1000), axes = FALSE,
        ylim = 0.5 + c(0, nrow(spot.list$spotdata)), yaxs = "i",
        xlab = "", ylab = "", zlim = max(abs(sample.spot.expression.image),
            na.rm = TRUE) * c(-1, 1))
    box()
    if (ncol(env$indata) < 100) {
        axis(1, 1:ncol(env$indata), labels = colnames(env$indata),
            las = 2, line = -0.5, tick = 0, cex.axis = 1.4)
    }
    ...
    plot(0, type = "n", xlab = "", ylab = "", axes = FALSE, xlim = c(0,
        1), ylim = 0.5 + c(0, nrow(spot.list$spotdata)), yaxs = "i")
    pos <- as.vector(sapply(c(1:nrow(spot.list$spotdata)), function(x) {
        c(x - 0.26, x, x + 0.26)
    }))
    text(0.05, rev(pos), unlist(lapply(spot.list$spots, function(x) {
        names(head(x$Fisher.p, 3))
    })), adj = 0, cex = 1)
    par(mar = c(5, 2, 4, 2))
    image(matrix(1:100, 100, 1), col = env$color.palette.heatmaps(1000),
        axes = FALSE, xlab = "")
    axis(1, round(max(max(sample.spot.expression.image), -min(sample.spot.expression.image)) *
        c(-1, 1), 1), at = c(0, 1), las = 2, tick = FALSE, pos = -0.8,
        cex.axis = 1.4)
    mtext(expression(paste("<", Delta, "e", ""^meta, ">")), side = 1,
        line = 0.5)
    box()
    for (m in seq_along(spot.list$spots)) {
        ...
        layout(matrix(c(1, 2, 4, 1, 3, 4, 5, 5, 6, 7, 7, 8),
        ...
        text(0.1, 0.66, paste("# genes =", length(spot.list$spots[[m]]$genes)),
            adj = 0)
        text(0.1, 0.55, paste("<r> metagenes =", round(mean(cor(t(env$metadata[spot.list$spots[[m]]$metagenes,
            ]))), 2)), adj = 0)
        if (length(spot.list$spots[[m]]$genes) < 1000) {
            suppressWarnings({
                try({
                  text(0.1, 0.51, paste("<r> genes =", round(mean(cor(t(env$indata[spot.list$spots[[m]]$genes,
                    ]))), 2)), adj = 0)
                }, silent = TRUE)
            })
        }
        text(0.1, 0.47, paste("beta: r2=", round(spot.list$spots[[m]]$beta.statistic$beta.score,
            2), " /  log p=", round(log10(spot.list$spots[[m]]$beta.statistic$beta.significance),
            2)), adj = 0)
        text(0.1, 0.39, paste("# samples with spot =", sum(sample.with.spot,
            na.rm = TRUE), "(", round(100 * sum(sample.with.spot,
            na.rm = TRUE)/ncol(env$indata), 1), "%)"), adj = 0)
        if (length(unique(env$group.labels)) > 1 && sum(sample.with.spot,
            na.rm = TRUE) > 0) {
            ...
            for (g in seq_along(group.table)) {
                text(0.15, 0.39 - g * 0.04, paste(names(group.table)[g],
                  ":", group.table[g], "(", round(100 * group.table[g]/sum(env$group.labels ==
                    names(group.table)[g]), 1), "%)"), adj = 0,
                  col = env$group.colors[match(names(group.table)[g],
                    env$group.labels)])
            }
        }
        par(mar = c(2, 3, 3, 1))
        ...
        image(matrix(spot.list$overview.map, env$preferences$dim.1stLvlSom,
            env$preferences$dim.1stLvlSom), axes = FALSE, col = col,
            main = "Overview Map", cex.main = 1.5)
        axis(1, seq(0, 1, length.out = env$preferences$dim.1stLvlSom/10 +
            1), c(1, seq(10, env$preferences$dim.1stLvlSom, length.out = env$preferences$dim.1stLvlSom/10)),
            cex.axis = 1)
        ...
        box()
        par(mar = c(8, 3, 1, 1))
        barplot(spot.list$spotdata[m, ], col = env$group.colors,
            main = "", names.arg = if (ncol(env$indata) < 100)
                colnames(env$indata)
            else rep("", ncol(env$indata)), las = 2, cex.main = 1,
            cex.lab = 1, cex.axis = 1, cex.names = 0.8, border = if (ncol(env$indata) <
                80)
                "black"
            else NA)
        box()
        if (length(spot.list$spots[[m]]$genes) > 0 && length(spot.list$spots[[m]]$genes) <
            5000) {
            ...
            e.max <- apply(env$indata[spot.list$spots[[m]]$genes,
                , drop = FALSE], 1, max, na.rm = TRUE)
            e.min <- apply(env$indata[spot.list$spots[[m]]$genes,
            ...
            text(x.coords[2], y.coords, paste(strwrap(trimmed_list, width=100), collapse="\n"), cex = 0.5, adj = 0)
        }
        else {
            frame()
        }
        ...
        if (env$preferences$activated.modules$geneset.analysis) {
            ...
            if (!is(try.res, "try-error")) {
                ...
                par(mar = c(3, 6, 2, 6))
                hist(spot.list$spots[[m]]$Fisher.p, bre = 20,
                  freq = FALSE, xlab = "p-value", ylab = "",
                  main = "p-values", las = 1, cex.main = 1.5,
                ...
                lines(spot.list$spots[[m]]$Fisher.p[o], Fdr.spot.list.samples[o],
                  lty = 2, lwd = 2)
                lines(spot.list$spots[[m]]$Fisher.p[o], fdr.spot.list.samples[o],
                  lty = 3, lwd = 3)
            ...
            n.sets <- 15
            n.cat <- length(unique(sapply(env$gs.def.list, function(x) {
                x$Type
            })))
        }
    }
